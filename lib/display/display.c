...
void writeMixedString(char* str) {
  for(int i = 0; i < 4; i++) {
    if(str[i] >= '0' && str[i] <= '9') {
      writeNumberToSegment(i, str[i] - '0');
    } else if((str[i] >= 'A' && str[i] <= 'Z') || (str[i] >= 'a' && str[i] <= 'z')) {
      writeCharToSegment(i, str[i]);
    } else {
      // Handle other characters if needed, for now we will turn off the segment
      writeCharToSegment(i, ' ');
    }
  }
}

void writeMixedStringAndWait(char* str, int delay){
  for (int i = 0; i < delay / 20; i++) {
    for(int j = 0; j < 4; j++) {
      if(str[j] >= '0' && str[j] <= '9') {
        writeNumberToSegment(j, str[j] - '0');
      } else if((str[j] >= 'A' && str[j] <= 'Z') || (str[j] >= 'a' && str[j] <= 'z')) {
        writeCharToSegment(j, str[j]);
      } else {
        // Handle other characters if needed, for now we will turn off the segment
        writeCharToSegment(j, ' ');
      }
      _delay_ms(5);
    }
  }
